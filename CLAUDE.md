...
- `src/snippetProvider.ts` (`PhpSnippetProvider`): `CompletionItemProvider` registrado para `language: 'php'` (em `src/extension.ts`). Lê a configuração `php-better-snippets.enable-symfony-snippets` (default `true`, contribuída em `package.json`) e pula as definições com `area: 'symfony'` quando estiver `false`. Para cada definição restante, monta o corpo final substituindo `$PHP_VARIABLE_TYPE`/`$PHP_FUNCTION_RETURN_TYPE`/`$PHP_POSSIBLE_EXCEPTIONS` (valores de `custom-variables.json`, importado direto como JSON) e, se o body contiver `NAMESPACE_MARKER`, substitui todas as ocorrências pelo namespace resolvido como **texto plano, escapado** (ou string vazia se não resolvido) — nunca por um tabstop, então o cursor nunca para no namespace. Snippets sem o marcador passam intactos. Cada `CompletionItem` define `item.range` explicitamente — sem isso, o VS Code calcula a área de substituição por conta própria e pode acabar selecionando o snippet inteiro após a expansão, em vez de focar o primeiro tabstop. Quando `document.getWordRangeAtPosition(position)` retorna um range (prefixo alfanumérico como `get`, `sg`), ele é usado **clipado no cursor** (`wordRange.start` → `position`) — o `wordRange` cobre a palavra inteira, então sem o clipping completar no meio de um identificador (`get` dentro de `getName`) substituiria o resto da palavra. Prefixos que começam com símbolo (`*`, `#`, `?`, `$t`, ...) nunca têm `wordRange`, pois esses caracteres são separadores de palavra; para esses, o provider casa manualmente o texto que precede o cursor na linha (`document.lineAt`) contra o prefixo de cada snippet (`matchingSymbolPrefixLength`) — só inclui o item se houver correspondência (evita listar símbolos não relacionados, tipo `#`/`?` ao digitar `*`) e ancora o range exatamente no trecho já digitado (evita duplicar o símbolo, tipo `##[...]`). Sem correspondência, o item só entra na lista se **não houver `wordRange`** e `context.triggerKind === CompletionTriggerKind.Invoke` (ex.: Ctrl+Space em linha vazia, onde todos os snippets aparecem com range vazio na posição do cursor); o guard `!wordRange` é necessário porque `Invoke` também é o kind reportado ao digitar caracteres de palavra e um item com range vazio nunca é filtrado pelo VS Code (a palavra de filtro fica vazia), o que faria os snippets simbólicos aparecerem ao digitar qualquer identificador.
- `src/extension.ts`: registra o provider — passando `NON_WORD_TRIGGER_CHARACTERS` (de `src/snippets.ts`) como trigger characters, já que o VS Code só invoca um `CompletionItemProvider` automaticamente ao digitar caracteres de palavra; essa lista contém apenas o **primeiro** caractere não-word de cada prefixo (`#`, `$`, `*`, `?`), pois um símbolo no meio do prefixo (o `=` de `$t=`) é digitado com a sessão de sugestões já aberta e registrá-lo abriria a widget após qualquer `=` do arquivo — e um `FileSystemWatcher('**/composer.json')` para invalidar o cache do resolver.
...
Suíte de testes unitários com **vitest** em `test/`: `namespaceResolver.test.ts` (resolução PSR-4, monorepo, cache por mtime e invalidação), `snippetProvider.test.ts` (substituição de custom-variables, substituição/omissão do marcador de namespace, snippets sem marcador, toggle de `enable-symfony-snippets`, ranges — clipping no cursor, prefixos simbólicos e mistos, filtragem por trigger kind), `snippets.test.ts` (`NON_WORD_TRIGGER_CHARACTERS`) e `extension.test.ts` (registro do provider com os trigger characters e invalidação do cache pelo watcher). Como `vscode` não existe como módulo real em runtime Node, `vitest.config.ts` faz alias de `vscode` para `test/mocks/vscode.ts` (mock mínimo de `Uri`, `CompletionItem`, `CompletionTriggerKind`, `SnippetString`, `workspace.getWorkspaceFolder`, `workspace.getConfiguration`, `workspace.createFileSystemWatcher`, `languages.registerCompletionItemProvider`). Além disso, testar manualmente: F5 no VS Code (`.vscode/launch.json` já roda `vscode:prepublish` como `preLaunchTask`) abre uma janela de Extension Development Host.
